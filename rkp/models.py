...
class FireObject(models.Model):
    ...
    height = models.FloatField(null=True, verbose_name='Растояние от пожарной нагрузки до нижнего пояса ферм, м')
    result = models.FloatField(blank=True, null=True, verbose_name='Площадь')
    q_sum = models.FloatField(blank=True, null=True, verbose_name='Суммареая пожарная нагрузка МДж')
    production_category = models.CharField(blank=True, max_length=10, verbose_name='Категория производств')
...
class Quantity(models.Model):
    ...
    material_Q = models.FloatField(blank=True, null=True, verbose_name='Пожарная нагрузка МДж')
    fire_load = models.ForeignKey(FireLoad, on_delete = models.CASCADE, blank=True, null=True)
    fire_object = models.ForeignKey(FireObject, on_delete = models.CASCADE, blank=True, null=True)
...
def rkp(instance):
    ...
    FireObject.objects.filter(id=instance.fire_object.id).update(production_category=production_category, q_sum=Q_SUM)
